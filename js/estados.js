...
function preload() {
  ...
  //BURBUJAS
  cantBur= random(2,4);
  burbujas = loadImage("assets/otros/burbujas.png");

      //SONIDOS
     soundBurbujas = loadSound("assets/sonidos/Burbujas.mp3");
      suma = loadSound("assets/sonidos/suma.mp3");
      resta = loadSound("assets/sonidos/resta.mp3");

      soundBurbujas.setVolume(0.3);
      suma.setVolume(0.2);
      resta.setVolume(0.2);
...
function draw() {
 let claro = color(103,173,196);
 let oscuro = color(22, 85, 104);
...
let red = map(count, 120, 1130, 103, 22);
let green = map(count, 120, 1130, 173, 85);
let blue = map(count, 120, 1130, 196, 104);
...
background(red,green,blue);
  ...
  if (estado == "tortugas") {
    if (r == 4) { //ESTO ES PARA EL DIALOGO
      n = 2;
      tiempo = 100;
      contDialogo = 300;
      r = 5;
  ...
  if (estado == "alimentar") {
    if (r == 5) {//ESTO ES PARA EL DIALOGO
      n = 3;
      tiempo = 100;
      contDialogo = 300;
      r = 6;
...
function mousePressed() { //APLICAMOS LA FUNCION OBJETO EN EL MOUSEPRESSED
  ...
  if (estado == "origen" || estado == "formacion" || estado == "mapa" || estado == "reciclaje"|| estado == "tortugas" || estado == "bifurcacion2" || estado == "alimentar" || estado == "bioacumulacion" || estado == "paso01" || estado == "paso02" || estado == "paso03") {
    Objeto(menu.px , menu.py , X(110) , Y(88), 200);
    Objeto(menu.px + X(120) , menu.py , X(110) , Y(88), 70);
    Objeto(menu.px + X(180) , menu.py , X(110) , Y(88), 20);
...
function Dialogo(dialogo_, posX_, posY_, tamX_, tamY_, xTexto_, yTexto_) {
  ...
  if(tiempo > 0 && cambiarPantalla == false){
    tiempo--;
  }
  ...
  if(tiempo <= 0){
    ...
    image(globoDialogo, posX, posY - transicion, tamX, tamY);
    fill(0);
    text(diag, xTexto, yTexto - transicion);
    pop();
...
function Transicionar(){
  ...
  if(cambiarPantalla == true){
    transicion +=6;
     count++;
  }else{
      transicion = transicion += 0;
      count = count += 0;
  }
  ...
  if(transicion >= 0 && transicion <= 3 ){
   ...
   if(cambiarPantalla == true && transicion < height ){
      for (var i = 0; i < cantBur; i++) {
     burbujas[i].dibujar();
   }
   }

   if(cambiarPantalla == true && transicion > 100 && transicion < 110){
     if (!soundBurbujas.isPlaying()) soundBurbujas.play();
     //background(0);
   }


//----TRANSICION PASOS BIFURCACION-----
if(cambiarPasos && atras == false){
transicionx  += 8;
}

if(cambiarPasos && atras == true){
transicionx  -= 8;
}

if(cambiarPasos == false){
  transicionx = transicionx += 0;
}

if(transicionx >= 0 && transicionx <= 3){
  cambiarPasos = false;
 transicionx = 4;
}
...
function Burbujas(){
this.x = random(width);
this.y = random(height+200, height+800);

    this.dibujar = function () {
    image(burbujas, this.x+random(3,-3), this.y-transicion*2, 59, 80);
}
}
