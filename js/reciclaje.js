class Reciclaje {
  ...
  Dibujar() {
    ...
    fill(255);
    textAlign(CENTER);
    text("Concientización consciente", displayWidth / 2, 100 - transicion);
    text(
    ...
    textSize(14);
    if (this.contador == 0 && this.jugar == false) {
      this.xBotellaP = X(650);
      this.yBotellaP = Y(700);
      this.xBotella = X(800);
      this.yBotella = Y(770);
    ...
    if (this.jugar == false) {
      image(this.tortuguita2, X(200), Y(800) - transicion, X(150), Y(290)); //INICIO
    }

    if (this.reciclar == "nada" && this.contador < 6 && this.jugar == true) {
      //NORMAL
      image(this.tortuguita, X(200), Y(800) - transicion, X(150), Y(290));
    } else if (
      ...
      //EQUIVOCARSE
      image(this.tortuguita3, X(200), Y(800) - transicion, X(150), Y(290));
    } else if (
      this.reciclar == "nada" &&
      this.contador == 6 &&
      this.jugar == true
      ...
      //GANAR {
      image(this.tortuguita4, X(200), Y(800) - transicion, X(150), Y(290));
      push();
      textAlign(LEFT);
      Dialogo(
    ...
    image(this.tachoRojo, X(800), Y(480) - transicion, X(220), Y(380)); //TACHO ROJO
    image(this.tachoAmarillo, X(1100), Y(480) - transicion, X(220), Y(380)); //TACHO AMARILLO
    image(this.tachoVerde, X(1400), Y(480) - transicion, X(220), Y(380)); //TACHO VERDE

    push();
    translate(this.xBotellaP, this.yBotellaP - transicion);
    rotate(300);
    image(this.botellaPlastico, 0, 0 , X(70), Y(140)); //BOTELLA PLASTICO
    pop();

    push();
    translate(this.xBotella, this.yBotella - transicion);
    rotate(25);
    image(this.botella, 0, 0, X(70), Y(140)); //BOTELLA VIDRIO
    pop();

    push();
    translate(this.xLata, this.yLata - transicion);
    rotate(45);
    image(this.lata, 0, 0, X(76), Y(155)); //LATA
    pop();

    push();
    translate(this.xPila, this.yPila - transicion);
    rotate(270);
    image(this.pila, 0, 0, X(57), Y(115)); //PILA
    pop();

    push();
    translate(this.xBolsa, this.yBolsa - transicion);
    rotate(25);
    image(this.bolsa, 0, 0, X(94), 191); //BOLSA
    pop();

    push();
    translate(this.xVidrios, this.yVidrios - transicion);
    rotate(25);
...
if(cambiarPantalla == false){
  ...
  DialogoReciclaje() {
    ...
    if (dialogo == "dialogoReciclaje1" && contDialogo <= 300 && transicion >= 0) {
      Dialogo(
    ...
    if (
      ...
      if(cambiarPantalla == false && tiempo == 0){
        push();
        imageMode(CORNER);
        image(this.btnjugar, X(230), Y(615) - transicion, X(140), Y(43));
        pop();
      }
    }
      pop();
